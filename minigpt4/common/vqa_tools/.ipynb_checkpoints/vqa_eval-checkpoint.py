...
llama_model_path = "/gemini/data-1/weights/vicuna_v0_7b"
class VQAEval:
    ...
    def evaluate(self, quesIds=None):
        ...
        for quesId in quesIds:
            ...
            #yes no 问题
            if(gtAns == 'yes' or gtAns == 'no'):
                accYesNo.append(gtAcc)
            #bleu_1 score
            bleu_1 = sentence_bleu(gtAns, resAns, weights=(1, 0, 0, 0))
            bleu1_avg += bleu_1
            self.ansComp[quesId] = [gtAns, resAns]

            #f1 score
            f1_avg += compute_f1(self.llama_tokenizer.encode(gtAns), self.llama_tokenizer.encode(resAns))

            # quesType = gts[quesId]['question_type']
            if 'answer_type' in gts[quesId]:
        ...
        self.setAccuracy(accQA, accQuesType, accAnsType,accYesNo,bleu1_avg,f1_avg)
    ...
    def setAccuracy(self, accQA, accQuesType, accAnsType,accYesNo,bleu1_avg,f1_avg):
        self.accuracy['overall'] = round(100 * float(sum(accQA)) / len(accQA), self.n)
        self.accuracy['bleu1_avg'] = round(100 * float(bleu1_avg) / len(accQA), self.n)
        self.accuracy['f1_avg'] = round(100 * float(f1_avg) / len(accQA), self.n)
        if len(accYesNo)==0:
            self.accuracy['yesno'] = 0
        else:
            self.accuracy['yesno'] = round(100 * float(sum(accYesNo)) / len(accYesNo), self.n)
