...
class MiniGPTBase(BaseModel):
    ...
    def get_context_emb(self, prompt, img_list):
        ...
        assert len(prompt_segs) == len(img_list) + 1, "图像占位符和图像数量不匹配"
        seg_tokens = [
    ...
    def preparing_embedding(self, samples):
        ...
        if 'conv_q' in samples:
            ...
            # if self.chat_template: #minigptv2的参数
    ...
    def embed_tokens(self, token_ids):
        if hasattr(self.llama_model.base_model, 'model') and hasattr(self.llama_model.base_model.model, 'model'): ## lora wrapped model
            embeds = self.llama_model.base_model.model.model.embed_tokens(token_ids)
        elif hasattr(self.llama_model.base_model, 'model'):
    ...
    def generate(
        ...
        temperature=1,
        do_sample=False,
        stop_words_ids=[2,13,396,444,835,2277, 29937],
    ):
        '''
        ...
        image_lists = [[image_emb[None]] for image_emb in img_embeds] #以None切片，增加维度
